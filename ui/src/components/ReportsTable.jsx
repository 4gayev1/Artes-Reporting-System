...
const CI_MAP = [
  ...
  ["local", { color: "#64748b", label: "Local Run" }],
];

function detectCI(executor, name, url) {
  const hay = ((executor || "") + (name || "") + (url || "")).toLowerCase();
  const hit = CI_MAP.find(([k]) => hay.includes(k));
  return hit
    ? hit[1]
    : { color: "#94a3b8", label: executor || name || "Pipeline" };
}
          ...
          return (
            <span
              ...
              style={{
                ...
                color,
                background: `${color}18`,
                border: `1px solid ${color}${hov === k ? "80" : "35"}`,
                borderRadius: 99,
                padding: "2px 7px",
                fontFamily: "monospace",
                opacity: val === 0 ? 0.25 : hov && hov !== k ? 0.35 : 1,
...
function PipelineCell({ status, executor, dark }) {
  const ci = detectCI(executor, status?.pipeline_name, status?.pipeline_url);
  const hasLink = status?.pipeline_url || status?.pipeline_name;
  const hasAny = hasLink || executor;
  if (!hasAny)
    return (
  ...
  const label = ps === 0 ? "passed" : ps === 1 ? "failed" : null;
    ...
    <>
      <span
        style={{
          ...
          flexShrink: 0,
          animation: ps === 0 ? "pulseDot 2s infinite" : "none",
        }}
      />
      <span
        style={{
      ...
      >
        <ExecutorIcon name={executor || status?.pipeline_name} size={13} />
        {ci.label}
      </span>
      {bn && (
        <span
      ...
      {hasLink && <FiExternalLink size={11} color={sc} />}
    </>
  );
...
function EnvCell({ environment }) {
  if (!environment)
    return <span style={{ color: "#475569", fontSize: 14 }}>—</span>;
  const s = envStyle(environment);
  return (
    <span
      style={{
        display: "inline-flex",
...
function OSCell({ os_name, browser_name, dark }) {
  const txtMuted = dark ? "#94a3b8" : "#64748b";
  const chipBg = dark ? "#0a1628" : "#f1f5f9";
  const chipBdr = dark ? "#1a2e4a" : "#e2e8f0";
  if (!os_name && !browser_name)
    return (
      <span style={{ color: dark ? "#475569" : "#94a3b8", fontSize: 14 }}>
        —
      </span>
  ...
  return (
    ...
    >
      ...
      {browser_name && (
        ...
        >
          <BrowserIcon name={browser_name} size={12} />
          {normalizeBrowser(browser_name)}
        </span>
      )}
    </div>
  );
          ...
          <tbody>
            {reports?.map((r) => (
              <tr key={r.id} className="tbl-tr">
                ...
                >
                  <EnvCell environment={r.environment} />
                </td>
